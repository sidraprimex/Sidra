...
export function CheckoutFlow({ userId }: { readonly userId: string }): React.JSX.Element {
  ...
  const applyCoupon = async (): Promise<void> => {
    ...
    setAppliedCoupon(null);
    try {
      const coupon = await validateSellerCoupon(
        couponCode,
        cart.items,
      );
  ...
  return <section className="grid gap-8">
    ...
    {step === 1 ? <div className="grid gap-5"><h2 className="font-heading text-3xl">Delivery and order review</h2>{draft.studioCount > 1 ? <div className="rounded-[var(--radius-lg)] border border-[var(--color-warning)] bg-card p-5 text-sm">Your order will be split into {draft.shipmentCount} shipments from {draft.studioCount} Studios.</div> : null}<div className="rounded-[var(--radius-lg)] border border-border bg-card p-6"><div className="flex justify-between"><span>Subtotal</span><span>{formatInr(draft.subtotalPaise)}</span></div><div className="mt-3 flex justify-between"><span>Shipping</span><span>{formatInr(draft.shippingPaise)}</span></div>{draft.discountPaise > 0 ? <div className="mt-3 flex justify-between text-emerald-700"><span>Coupon {draft.couponCode}</span><span>−{formatInr(draft.discountPaise)}</span></div> : null}<div className="mt-5 flex justify-between border-t border-border pt-5 font-heading text-2xl"><span>Total</span><span>{formatInr(draft.totalPaise)}</span></div></div><div className="grid gap-3 rounded-[var(--radius-md)] border border-border bg-card p-4"><label className="grid gap-2 text-sm font-semibold">Seller coupon code<div className="flex flex-col gap-3 sm:flex-row"><input value={couponCode} onChange={(event) => { setCouponCode(event.target.value.toUpperCase()); setCouponMessage(null); }} className="min-w-0 flex-1 rounded-[var(--radius-md)] border border-border bg-white px-4 py-3" placeholder="Example: ZARA10" /><button type="button" disabled={couponBusy || couponCode.trim().length < 3} onClick={() => void applyCoupon()} className="rounded-[var(--radius-md)] bg-[var(--color-deep-plum)] px-5 py-3 text-white disabled:opacity-50">{couponBusy ? "Checking…" : appliedCoupon ? "Reapply" : "Apply coupon"}</button>{appliedCoupon ? <button type="button" onClick={() => { setAppliedCoupon(null); setCouponCode(""); setCouponMessage(null); }} className="rounded-[var(--radius-md)] border border-border px-5 py-3">Remove</button> : null}</div></label>{couponMessage ? <p className={`text-sm ${appliedCoupon ? "text-emerald-700" : "text-red-700"}`}>{couponMessage}</p> : null}<p className="text-sm text-muted">A seller coupon only discounts eligible products from that seller. It never changes another Studio&apos;s order.</p></div><button className="justify-self-start rounded-[var(--radius-md)] bg-[var(--color-gold-600)] px-5 py-3 text-white" onClick={() => setStep(2)}>Continue to payment</button></div> : null}

    {step === 2 ? <div className="grid gap-6"><h2 className="font-heading text-3xl">Payment</h2>{paymentSettings.mode === "disabled" ? <div className="rounded-[var(--radius-lg)] border border-[var(--color-warning)] bg-card p-6"><h3 className="font-heading text-2xl">Payments are temporarily paused</h3><p className="mt-3 text-sm leading-7 text-muted">Contact {paymentSettings.supportContact || "Sidra Support"} before placing this order.</p></div> : null}{razorpayVisible && paymentSettings.razorpayEnabled ? <div className="rounded-[var(--radius-lg)] border border-border bg-card p-6"><p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--color-gold-600)]">Razorpay</p><h3 className="mt-2 font-heading text-3xl">Secure hosted checkout</h3><p className="mt-3 max-w-2xl leading-7 text-muted">SIDRA never receives or stores raw card details. Payment is handled by the gateway.</p><button disabled={processing} className="mt-5 rounded-[var(--radius-md)] bg-[var(--color-gold-600)] px-5 py-3 text-white disabled:opacity-50" onClick={() => void payWithRazorpay()}>{processing ? "Opening secure checkout…" : `Pay ${formatInr(draft.totalPaise)}`}</button></div> : null}{manualVisible && paymentSettings.manualEnabled ? <div className="rounded-[var(--radius-lg)] border border-border bg-card p-6"><p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--color-gold-600)]">Manual verification</p><h3 className="mt-2 font-heading text-3xl">Pay by UPI or bank transfer</h3><div className="mt-5 grid gap-5 md:grid-cols-[1fr_auto]"><dl className="grid content-start gap-3 text-sm sm:grid-cols-2"><div><dt className="font-semibold">UPI ID</dt><dd className="mt-1 break-all text-muted">{paymentSettings.upiId || "Not configured"}</dd></div><div><dt className="font-semibold">Account holder</dt><dd className="mt-1 text-muted">{paymentSettings.accountHolderName || "Not configured"}</dd></div><div><dt className="font-semibold">Bank</dt><dd className="mt-1 text-muted">{paymentSettings.bankName || "Not configured"}</dd></div><div><dt className="font-semibold">Payment support</dt><dd className="mt-1 break-all text-muted">{paymentSettings.supportContact || "9019254743"}</dd></div></dl><UpiPaymentQr upiId={paymentSettings.upiId} payeeName={paymentSettings.accountHolderName || "Sidra"} amountPaise={draft.totalPaise} reference="Sidra marketplace order" /></div>{paymentSettings.instructions ? <p className="mt-5 rounded-[var(--radius-md)] border border-border p-4 text-sm leading-7 text-muted">{paymentSettings.instructions}</p> : null}<label className="mt-5 grid gap-2 text-sm font-semibold">UTR / transaction reference<input value={manualReference} onChange={(event) => setManualReference(event.target.value)} className="rounded-[var(--radius-md)] border border-border bg-white px-4 py-3" placeholder="Example: 312345678901" /></label><button disabled={processing} className="mt-5 rounded-[var(--radius-md)] bg-[var(--color-deep-plum)] px-5 py-3 text-white disabled:opacity-50" onClick={() => void submitManualPayment()}>{processing ? "Submitting…" : "Submit for admin verification"}</button></div> : null}{error ? <div className="rounded-[var(--radius-md)] border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</div> : null}</div> : null}
