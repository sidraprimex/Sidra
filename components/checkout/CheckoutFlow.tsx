...
export function CheckoutFlow({ userId }: { readonly userId: string }): React.JSX.Element {
  ...
  return <section className="grid gap-8">
    ...
    {step === 1 ? <div className="grid gap-5"><h2 className="font-heading text-3xl">Delivery and order review</h2>{draft.studioCount > 1 ? <div className="rounded-[var(--radius-lg)] border border-[var(--color-warning)] bg-card p-5 text-sm">Your order will be split into {draft.shipmentCount} shipments from {draft.studioCount} Studios.</div> : null}<div className="rounded-[var(--radius-lg)] border border-border bg-card p-6"><div className="flex justify-between"><span>Subtotal</span><span>{formatInr(draft.subtotalPaise)}</span></div><div className="mt-3 flex justify-between"><span>Shipping</span><span>{formatInr(draft.shippingPaise)}</span></div><div className="mt-5 flex justify-between border-t border-border pt-5 font-heading text-2xl"><span>Total</span><span>{formatInr(draft.totalPaise)}</span></div></div><div className="rounded-[var(--radius-md)] border border-border bg-card p-4 text-sm text-muted">Coupons are coming in Phase 12. No fake coupon discount is applied.</div><button className="justify-self-start rounded-[var(--radius-md)] bg-[var(--color-gold-600)] px-5 py-3 text-white" onClick={() => setStep(2)}>Continue to payment</button></div> : null}
