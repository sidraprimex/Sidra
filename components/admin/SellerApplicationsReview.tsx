...
export function SellerApplicationsReview() {
  ...
  const pending = useMemo(() => items.filter((item) => ["pending", "moreInfoRequested", "onHold", "provisioningFailed"].includes(item.status)), [items]);
  const act = async (item: SellerApplication, decision: SellerApplicationDecision) => {
    const note = notes[item.id]?.trim() ?? "";
    if (decision !== "approve" && note.length < 3) { setError("Add a clear review note before this decision."); return; }
  ...
  if (pending.length === 0) return <EmptyState title="No Studio requests waiting" message="New verified artisan requests will appear here for Founder review." />;
  return <div className="grid gap-6">{error ? <ErrorState message={error} onRetry={() => setError(null)} /> : null}{pending.map((item) => <Card key={item.id} elevated className="grid gap-5">
    ...
    <section><h3 className="font-display text-h3">Portfolio</h3><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">{item.portfolioImages.map((image) => <a key={image.path} href={image.downloadUrl} target="_blank" rel="noreferrer" className="group relative aspect-square overflow-hidden rounded-sm border border-gray-100 bg-gray-100" aria-label={`Open ${image.fileName}`}><span className="absolute inset-0 bg-cover bg-center transition duration-base ease-luxury group-hover:scale-105" style={{ backgroundImage: `url(${image.downloadUrl})` }} /><span className="absolute inset-x-0 bottom-0 bg-black-900/70 p-2 text-micro text-ivory-50">Open image</span></a>)}</div></section>
    <label className="grid gap-2 text-caption font-semibold">Founder review note<textarea className="min-h-24 rounded-sm border border-gray-300 bg-ivory-50 p-4 text-body font-normal" value={notes[item.id] ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} /></label>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Button loading={working === `${item.id}:approve`} onClick={() => void act(item, "approve")}>Approve</Button><Button variant="outline" loading={working === `${item.id}:requestMoreInfo`} onClick={() => void act(item, "requestMoreInfo")}>Request more info</Button><Button variant="ghost" loading={working === `${item.id}:hold`} onClick={() => void act(item, "hold")}>Hold</Button><Button variant="danger" loading={working === `${item.id}:reject`} onClick={() => void act(item, "reject")}>Reject</Button></div>
  </Card>)}</div>;
}
