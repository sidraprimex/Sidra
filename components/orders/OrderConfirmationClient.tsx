...
export function OrderConfirmationClient({ orderId }: { readonly orderId: string }): React.JSX.Element {
  ...
  return <section className="grid gap-8"><header><p className="text-xs uppercase tracking-[0.18em] text-[var(--color-success)]">Payment verified</p><h1 className="mt-3 font-heading text-[clamp(3rem,8vw,6rem)]">Order confirmed</h1><p className="mt-4 text-lg text-muted">{order.orderNumber}</p></header><div className="rounded-[var(--radius-lg)] border border-border bg-card p-6">{order.items.map((item) => <div key={`${item.productId}-${item.variantId ?? "default"}`} className="flex justify-between border-b border-border py-4 last:border-0"><span>{item.productName} × {item.quantity}</span><span>{formatInr(item.unitPricePaise * item.quantity)}</span></div>)}<div className="mt-5 flex justify-between border-t border-border pt-5 font-heading text-2xl"><span>Total</span><span>{formatInr(order.totalPaise)}</span></div></div><div className="flex flex-wrap gap-3"><a href={order.invoiceUrl} className="rounded-[var(--radius-md)] border border-border px-5 py-3">Download invoice</a><a href={`/account/orders/${order.orderId}`} className="rounded-[var(--radius-md)] bg-[var(--color-gold-600)] px-5 py-3 text-white">Track order</a></div></section>;
}
