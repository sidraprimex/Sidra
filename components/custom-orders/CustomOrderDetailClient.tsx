...
import { canCustomerPayCustomOrder, canSellerSendQuote, canSubmitProof } from "@/utils/customOrderLifecycle";
...
export function CustomOrderDetailClient({
  ...
  return <section className="grid gap-8">
    ...
    {order.quote ? <CustomOrderQuoteCard customOrderId={order.customOrderId} quote={order.quote} canAccept={role === "customer" && canCustomerPayCustomOrder(order.status)} /> : null}
    ...
    <CustomOrderConversation customOrderId={order.customOrderId} messages={order.messages} />
  </section>;
}
