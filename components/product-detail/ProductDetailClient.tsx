...
import { addCartItem, replaceCartForBuyNow, safeCartQuantity } from "@/services/cartSyncService";
...
export function ProductDetailClient({ product }: { readonly product: PublicProduct }): React.JSX.Element {
  ...
  useEffect(() => {
    if (!auth.user) { setWishlisted(false); return; }
    void isProductWishlisted(auth.user.uid, product.id).then(setWishlisted).catch(() => setWishlisted(false));
  ...
  const addToCart = async (checkout: boolean) => {
    ...
    try {
      const item = {
        ...
        quantity: safeCartQuantity(quantity),
      ...
      if (checkout) await replaceCartForBuyNow(auth.user.uid, item);
      else await addCartItem(auth.user.uid, item);
  ...
  const toggleWishlist = async () => {
    ...
    try {
      const result = await toggleWishlistProduct({
        productId: product.id, productSlug: product.slug, productName: product.name, imageUrl: activeImage,
        studioId: product.studioId, studioName: product.studioName ?? "Sidra Studio",
        pricePaise: product.salePricePaise ?? product.pricePaise,
      });
      setWishlisted(result.active);
      setMessage(result.active ? "Saved to your wishlist." : "Removed from your wishlist.");
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Wishlist could not be updated."); }
