...
import { SELLER_PLANS, type SellerSubscriptionPlan } from "@/types/seller-subscription";
...
export function NewProductController({ existingProductId }: { readonly existingProductId?: string }): React.JSX.Element {
  ...
  return <ProductForm initialValue={initialValue ?? undefined} categories={categories} collections={collections} media={media} saving={saving} subscriptionPlan={plan} commissionBasisPoints={commissionBasisPoints} planLabel={planLabel} onUpload={async (files,input) => { if (!auth.claims?.studioId) throw new Error("Your verified Studio connection is required."); const safeInput={...input,costing:{makingCostPaise:input.costing?.makingCostPaise??0,sellerShippingCostPaise:0}}; const existed=Boolean(productId); const id=await ensure(safeInput); if(existed)await updateProductDraft(id,safeInput); setMedia(await uploadProductImages(auth.claims.studioId,id,files)); }} onSave={async (input, intent) => { setSaving(true); try { const safeInput={...input,costing:{makingCostPaise:input.costing?.makingCostPaise??0,sellerShippingCostPaise:0}}; const existingId = productId; const id = await ensure(safeInput); if (existingId) await updateProductDraft(id, safeInput); if (intent === "submit") await submitProduct(id); } finally { setSaving(false); } }} />;
}
